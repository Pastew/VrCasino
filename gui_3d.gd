...
func _ready():
	...
	# Setup user input
	var controller = get_node("/root/VrCasino/XROrigin3D/RightController");
	controller.button_pressed.connect(self._on_left_controller_button_pressed)
	controller.button_released.connect(self._on_left_controller_button_released)
...
func get_spin_button_position():
	...
	var y = node_viewport.size.y * 0.85
	var pos = Vector2(x, y)
